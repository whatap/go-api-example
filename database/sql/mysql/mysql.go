...
func main() {
	...
	http.HandleFunc("/", whataphttp.Func(func(w http.ResponseWriter, r *http.Request) {
		...
		tp, err := template.ParseFiles("templates/database/sql/mysql/index.html")
		if err != nil {
			fmt.Println("Template not loaded, ", err)
			return
		}
		data := &HTMLData{}
		data.Title = "database/sql/mysql server"
	...
	http.HandleFunc("/query/error", whataphttp.Func(func(w http.ResponseWriter, r *http.Request) {
		var buffer bytes.Buffer
		w.Header().Add("Content-Type", "text/html")
		...
		var id int
		var subject string
		rows, err := db.Query("select id, subject from tbl_faq limit $1")
		if err != nil {
			fmt.Println("Error db.QueryContext ", err)
			return
		}
	...
	http.HandleFunc("/tx/rollback", whataphttp.Func(func(w http.ResponseWriter, r *http.Request) {
		...
		if tx, err := db.BeginTx(ctx, nil); err == nil {
			...
			query = "select id, subject fr tbl_faq where id in ($1,$2)"
