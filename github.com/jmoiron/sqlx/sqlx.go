...
type Person struct {
	Id     int        `db:"id"`
	Name   string     `db:"name"`
	Jumin  string     `db:"jumin"`
	Adm    *time.Time `db:"adm"`
	Retire *time.Time `db:"retire"`
}
...
func main() {
	...
	wDB, err := whatapsql.OpenContext(context.Background(), PGSQL_DRIVER_NAME, dataSource)
	if err != nil {
	...
	serviceDB := sqlx.NewDb(wDB, PGSQL_DRIVER_NAME)
	defer serviceDB.Close()

	http.HandleFunc("/", whataphttp.Func(func(w http.ResponseWriter, r *http.Request) {
		fmt.Println("Request -", r)

		tp, err := template.ParseFiles("templates/github.com/jmoiron/sqlx/index.html")
		if err != nil {
	...
	http.HandleFunc("/query", whataphttp.Func(func(w http.ResponseWriter, r *http.Request) {
		...
		wdb, err := whatapsql.OpenContext(ctx, PGSQL_DRIVER_NAME, dataSource)
		if err != nil {
		...
		if err != nil {
			fmt.Println("Error db.Select to person ", err)
			return
		}
		for _, it := range person {
			fmt.Println(it.Id, it.Name)
			buffer.WriteString(fmt.Sprintln(it.Id, it.Name, "<br>"))
		}
	...
	http.HandleFunc("/queryContext", whataphttp.Func(func(w http.ResponseWriter, r *http.Request) {
		var buffer bytes.Buffer
		w.Header().Add("Content-Type", "text/html")
	...
	http.HandleFunc("/service/select", whataphttp.Func(func(w http.ResponseWriter, r *http.Request) {
		var buffer bytes.Buffer
		w.Header().Add("Content-Type", "text/html")
	...
	http.HandleFunc("/service/selectContext", whataphttp.Func(func(w http.ResponseWriter, r *http.Request) {
		var buffer bytes.Buffer
		w.Header().Add("Content-Type", "text/html")
	...
	http.HandleFunc("/notx/select", whataphttp.Func(func(w http.ResponseWriter, r *http.Request) {
		var buffer bytes.Buffer
		w.Header().Add("Content-Type", "text/html")
	...
	http.HandleFunc("/lost/onlyStart", func(w http.ResponseWriter, r *http.Request) {
		// ctx := r.Context()
		trace.StartWithRequest(r)
	...
	http.HandleFunc("/lost/onlyEnd", func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		defer trace.End(ctx, nil)
	...
	http.HandleFunc("/lost/start2End1", func(w http.ResponseWriter, r *http.Request) {
	...
	http.HandleFunc("/lost/start1End2", func(w http.ResponseWriter, r *http.Request) {
		ctx := r.Context()
		defer trace.End(ctx, nil)
